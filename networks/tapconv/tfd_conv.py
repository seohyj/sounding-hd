# AV-HD/networks/tapconv/tfd_conv.py
import torch
...
class Dynamic_conv2d(nn.Module):
    def __init__(self, in_planes, out_planes, freq_size, kernel_size, stride=1, padding=0, groups=1, bias=False,
                 n_basis_kernels=4, temperature=31, reduction=4, pool_dim='freq', pool_type='avg', conv1d_kernel=[3, 1],
        ...
        if dy_chan_proportion is not None: # PFD Conv (Partial Freq Dynamic Conv) / MDFD Conv (Multi-Dilated Freq Dynamic Conv)
            # dilation size = Number of dynamic branches
            self.n_attention = len(dilation_size)
            # dynamic branch: dy_out_planes computes num of channel that each dynamic branch will take
            dy_out_planes = int(out_planes * dy_chan_proportion[0] / dy_chan_proportion[1])
            self.dy_out_planes = dy_out_planes
            # static branch = total out_planes - dynamic channel
            # (note that: total num of out channels = static + (dynamic channel * dynamic branch))
            self.stt_out_plane = out_planes - dy_out_planes * self.n_attention

            # DFD Conv (Dilated Freq Dynamic Conv): Controlled with dilated_DY and dilation_size
            ...
            if not aggconv:
                if self.stt_out_plane > 0:
                    # Conventional 2D convolution for static branch
            ...
            # Attention module for each dynamic branch.
            # Generates n_attention number of attention2d instance and append to self.attentions list
            # Each dynamic branch computes its own attention weights
            self.attentions = []
            ...
            for i in range(self.n_attention):
                if i == 0:
                    self.attention_0 = attention2d(in_planes, conv1d_kernel, freq_size, self.stride,
                                                   n_basis_kernels[i], temperature, reduction, pool_dim, pool_type)
            ...
            self.attention = attention2d(in_planes, conv1d_kernel, freq_size, self.stride,
                                         n_basis_kernels, temperature, reduction, pool_dim, pool_type)
    ...
    def forward(self, x):                                           # x size : [bs, in_chan, frames, freqs]
        if self.dy_chan_proportion is not None:
            if not self.aggconv: 
                if self.stt_out_plane: # aggconv False -> just normal 2D Conv for static branch
                    stt_output = self.stt_conv(x)
                att_outputs = ()

                # Use each attention module stored in self.attentions.
                # Call attention_forward to compute the attention-weighted output for each dynamic branch,
                # passing that branch's dilation_size and weight as parameters.
                for i in range(self.n_attention):
                    bias = self.bias[i] if self.bias is not None else None
                    # DFD Conv is applied by dilation_size[i] if self.dilated_DY is True
            ...
            # Final output concatenation along the channel axis (dim=1)
            if self.stt_out_plane > 0:
                output = torch.cat((stt_output,) + att_outputs, dim=1)
            else: # if no static branch = DFD Conv
    ...
    # Frequency Adaptive Kernel
    def attention_forward(self, x, attention, dilation_size, weight, bias):
        # 1. Compute attention weights for each frequency band
        # kernel_attention size : [bs, n_ker, 1, 1, freqs]
        kernel_attention = attention(x)                
        
        # 2. Apply convolution with basis kernels
        ...
        # 3. Apply Freq Adaptive Kernel: apply attention weights to the output
        output = torch.sum(output * kernel_attention, dim=1)  # output size : [bs, out_chan, frames, freqs]
...
class attention2d(nn.Module):
    def __init__(self, in_planes, kernel_size, freq_size, stride, n_basis_kernels,
                 temperature, reduction, pool_dim, pool_type):
        ...
        # time pooling
        if self.pool_type in ["SE2D", "SE2D_xforward", "SE2D_xf_res", "SE2D_xf_diff",
                              "SE2D_xf_resdiff", "SE2D_xf_all", "SE2D_xf_xdiff"]:
            """
            encoder type = SE2D - use only TA pooling
            encoder type = SE2D_xf_diff - use only VA pooling
            encoder type = SE2D_xf_res - use TA + AVG pooling
            encoder type = SE2D_xf_resdiff - use VA + AVG pooling
            encoder type = SE2D_xf_all - use TA + VA + AVG pooling
            """
            self.time_pool = AttentionPool(in_planes, encoder_type=self.pool_type)
...
class AttentionPool(nn.Module):
    def __init__(self, dim, kernel_size=1, encoder_type="ASP", reduction=4, hid_dim_lim=4):
        super().__init__()
        self.encoder_type = encoder_type   # ASP or SAP

        hid_dim = dim // reduction
        if hid_dim < hid_dim_lim:
            hid_dim = hid_dim_lim

        if self.encoder_type in ["SE2D_xforward", "SE2D_xf_res", "SE2D_xf_diff", "SE2D_xf_resdiff", "SE2D_xf_all",
                                   "SE2D_xf_xdiff"]:
            self.xforward = nn.Sequential(
                nn.Conv2d(dim, hid_dim, kernel_size=3, padding=1),
                nn.BatchNorm2d(hid_dim),
                nn.ReLU(),
                nn.Conv2d(hid_dim, dim, kernel_size=1))


        if self.encoder_type in ["SE2D", "SE2D_xforward", "SE2D_xf_res", "SE2D_xf_diff", "SE2D_xf_resdiff",
                                 "SE2D_xf_all", "SE2D_xf_xdiff"]:
            ...
            self.attention = nn.Sequential(
                nn.Conv1d(dim, hid_dim, kernel_size=kernel_size),
                nn.BatchNorm1d(hid_dim),
                nn.ReLU(),
                nn.Conv1d(hid_dim, dim, kernel_size=kernel_size),
                nn.Softmax(dim=2),)

        if self.encoder_type in ["SE2D_xf_all", "SE2D_xf_xdiff"]:
    ...
    def forward(self, x):                     # x size : [bs, chan, frame, freq]
        if self.encoder_type in ["SE2D", "SE2D_xforward", "SE2D_xf_res", "SE2D_xf_diff", "SE2D_xf_resdiff",
                                 "SE2D_xf_all", "SE2D_xf_xdiff"]:

            # ** Velocity Attention Pooling **
            if self.encoder_type in ["SE2D_xf_diff", "SE2D_xf_resdiff", "SE2D_xf_all", "SE2D_xf_xdiff"]:
                # Adjacent-frame difference along the time axis (dim 2 of [bs, chan, frame, freq]).
                # torch.roll would wrap the last frame around onto the first one, making the very
                # first frame's "velocity" an artificial end-to-start transition; the first frame
                # simply has no predecessor, so its difference is zero.
                x_diff = torch.zeros_like(x)
                x_diff[:, :, 1:] = torch.abs(x[:, :, 1:] - x[:, :, :-1])
                att = torch.softmax(self.attention(x_diff), dim=2)        # att size : [bs, chan, frame, freq]
            
            # ** Time Attention Pooling **
            else:
                att = torch.softmax(self.attention(x), dim=2)        # att size : [bs, chan, frame, freq]

            if self.xforward is None:
                x_fwd = x
            else:
                x_fwd = torch.sigmoid(self.xforward(x))          #### maybe different xforward for diff

            # out = torch.sum(x_fwd * att, dim=2) -- weighted sum pooling (either TA or VA)
            if self.encoder_type in ["SE2D_xf_all", "SE2D_xf_xdiff"]:
                att2 = torch.softmax(self.attention2(x), dim=2)        # att size : [bs, chan, frame, freq]
                out = torch.sum(x_fwd * att, dim=2)           # x size : [bs, chan, freq]
                x2 = torch.sum(x_fwd * att2, dim=2)           # x size : [bs, chan, freq]
                out += x2
            else:
                out = torch.sum(x_fwd * att, dim=2)           # x size : [bs, chan, freq]

            # ** Average Pooling **
            if self.encoder_type in ["SE2D_xf_res", "SE2D_xf_resdiff", "SE2D_xf_all"]:
                x_mean = torch.mean(x, dim=2) # average for temporal-axis
                out += x_mean # add to the prev. result ('out')
...
class DYCNN(nn.Module):
    def __init__(self,
                 ...
                 n_filt=[64, 64, 64],
                 pooling=[(1, 4), (1, 4), (1, 4)],
                 n_mels=128,  # Add n_mels to dynamically calculate frequency dimensions
                 pre_conv=None,
                 normalization="batch",
        ...
        # Dynamically calculate frequency dimensions based on pooling layers
        freq_dims = []
        current_freq_dim = n_mels
        for pool_kernel in pooling:
            # Append the frequency dimension *before* this layer's pooling
            freq_dims.append(current_freq_dim)
            # Calculate the frequency dimension *after* this layer's pooling
            # Use math.ceil to handle non-integer divisions
            current_freq_dim = math.ceil(current_freq_dim / pool_kernel[1])

        # Check if the length of freq_dims matches n_filt
        if len(freq_dims) != len(n_filt):
            raise ValueError(f"Length of pooling ({len(pooling)}) must match the number of filters ({len(n_filt)}).")
        ...
        def conv(i, normalization="batch", dropout=None, activ='relu'):
            ...
            # Use the dynamically calculated frequency dimension for this layer
            layer_freq_dim = freq_dims[i]
            
            # Add convolution layer
            if DY_layers[i] == 1:
                # Use the calculated freq_dim for Dynamic_conv2d
                cnn.add_module("conv{0}".format(i), Dynamic_conv2d(
                    in_dim, out_dim, layer_freq_dim, kernel[i], stride[i], pad[i],
                ...
                cnn.add_module("conv{0}".format(i), nn.Conv2d(
                    in_dim, out_dim, kernel[i], stride[i], pad[i], dilation[i]))
            ...
            if activ.lower() == "relu":
                ...
                # Assuming GLU class is defined elsewhere
                cnn.add_module("glu{0}".format(i), GLU(out_dim))
            elif activ.lower() == "cg":
                # Assuming ContextGating class is defined elsewhere
